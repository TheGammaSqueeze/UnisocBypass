#!/usr/bin/env python3
"""Exp 026: push CPU to 2150 MHz on top OPP, keep everything else at exp 025 baseline.

Stepping up from exp 025 (2100 MHz stable) by 50 MHz. CPU overvolt does NOT
apply via DT (exp 019 confirmed), so keep stock 1050 mV target. If 2150 MHz
is stable at the hardware-enforced ~1050 mV ceiling, try exp 027 at 2200.
If 2150 crashes, we've found the CPU freq ceiling at stock voltage.
"""
import pathlib
...
# GPU OC 900 MHz at stock 800 mV (same as exp 020/025)
...
print(f'  CPU: lower OPPs UV -{cpu_uv} mV, top OPP {top_mhz} MHz @ {top_mv} mV target')
